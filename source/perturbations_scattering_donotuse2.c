...
/* NEW: Neutrino scattering rate fit - file-scope struct and function */
struct rate_fit_coefs {
  double a0, a1, a2; /* Numerator constant terms */
  double b0, b1, b2; /* Numerator log(a) terms */
  double c0, c1, c2; /* Denominator log(a) terms */
};

static double evaluate_fitted_rate(double log_a, double m, double g, double conv_fact, struct rate_fit_coefs c) {
  double num = (c.a0 + c.a1*m + c.a2*m*m) + (c.b0 + c.b1*m + c.b2*m*m) * log_a;
  double den = 1.0 + (c.c0 + c.c1*m + c.c2*m*m) * log_a;
...
int perturbations_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    if (pba->has_ur == _TRUE_) {
      ...
      /* NEW: Relativistic neutrino decay parameters: */
      /* Declare all locals at top of block (C89 compatibility) */
      double m, conv_fact, YineV, delmsq, Phi, k_fit, rho_nu_by_rho_total;
      double Mterm, Massratio, g, rho_phi_by_rho_total, log_a;
      double Gamma_nunu_to_phiphi, Gamma_nuphi_to_nuphi, Gamma_nunu_to_nunu, Gamma_phiphi_to_nunu;
      double Gamma_nu_nu, Gamma_nu_phi, Gamma_phi_nu, Gamma_ann, Gamma_crea, scat_tot;
      struct rate_fit_coefs coef_ann, coef_scat, coef_self, coef_crea;
      capX = ppt->capX; /* mother neutrino mass divided by neutrino temperature */
      capY = ppt->capY; /* neutrino decay rate including mass dependent terms and phase space factor */
      m = (capX*0.05)/298.0; /* neutrino mass in eV */
      conv_fact = 1.56374e29;
      YineV = capY*6.395*pow(10,-30); /* capY is in Mpc^-1, Converted to eV for consistency */
      delmsq = 7.5e-5; /* eV^2 */
      Phi = (1.0/3.0)*(delmsq/(m*m))*(delmsq/(m*m));
      k_fit = (6.55*pow(10,10))/3.0;
      rho_nu_by_rho_total = 1.0/3.0;
      Mterm = m-sqrt((m*m-delmsq));
      Massratio= pow((m/0.05),5.0);
      g = sqrt((YineV*4.0*_PI_*pow(m,3.0))/(k_fit*Phi*Massratio*delmsq*Mterm*Mterm));
...
if (g_printed == 0) {
    printf("\n========================================================\n");
    printf("   DYNAMIC NEUTRINO COUPLING TRANSLATION DIAGNOSTIC\n");
    printf("========================================================\n");
    printf("-> Scale Factor at Init (a)   = %e\n", a);
    printf("-> Input Decay Rate (capY)    = %e Mpc^-1\n", capY);
    printf("-> Converted Decay (YineV)    = %e eV\n", YineV);
    printf("-> Target Mass Param (m)      = %e eV\n", m);
    printf("-> Mass Split Term (Mterm)    = %e eV\n", Mterm);
    printf("-> Tracking Phase Space (Phi) = %e\n", Phi);
    printf("-------------------------------------------------------\n");
    printf("-> DERIVED RUNTIME COUPLING g = %e\n", g);
    printf("-> Expected g^4 scaling factor = %e\n", pow(g, 4.0));
    printf("========================================================\n\n");
    fflush(stdout);
    g_printed = 1;
}*/
...
/*if (a < 1e-4) {
    double conformal_hub = a_prime_over_a; // This is H * a in Mpc^-1
    double conformal_scat = a * scat_tot;  // scattering rate in conformal time  
    printf("\n--- DIAGNOSTIC: Scale Factor a = %e (log_a = %e) ---\n", a, log_a);
    printf("Neutrino Mass m = %e eV | Coupling g = %e\n", m, g);
    printf("1. PHYSICAL HUBBLE RATE (H)          : %e Mpc^-1\n", conformal_hub / a);
    printf("2. CONFORMAL HUBBLE RATE (aH)        : %e Mpc^-1\n", conformal_hub);
    printf("3. CONFORMAL SCATTERING RATE (a*Gamma): %e Mpc^-1\n", conformal_scat);
    
    // Calculate the absolute stiffness ratio
    printf("-> STIFFNESS RATIO (a*Gamma / aH)    : %e\n", conformal_scat / conformal_hub);
    
    if (conformal_scat / conformal_hub > 1e10) {
        printf("!! WARNING: Integrator is trapped. Scattering is %e times faster than expansion !!\n", conformal_scat / conformal_hub);
    }
    printf("-------------------------------------------------------\n");
}*/
      ...
      if (capX > 0.0){
         ...
         // Smooth function with right behaviour at small x and large x
  curlyF = exp(-pow(a*capX,4)/16) * (- 1./2. + 2.*a*capX + (0.57721+log(a*capX))*(pow(a*capX,2)/2-1) - pow(a*capX,2) - pow(a*capX,3)/9 + 
        ...
        if(ppw->approx[ppw->index_ap_ufa] == (int)ufa_off) {
          double conformal_scat, decay_damp;
          (void)0; /* declarations above, assignments below */
          conformal_scat = a * scat_tot;
          decay_damp     = pow(a,6)*capY*curlyF;
          /* active_scat reserved for future use (currently commented out) */
          // If scattering is more than 1 million times faster than Hubble (a_prime_over_a),
          // it is in a runaway, unstable numerical regime. Keep it OFF to protect the solver.   
          /*if (conformal_scat > 1e8 * a_prime_over_a) {
              active_scat = 0.0; 
          } 
          // Otherwise, the universe has expanded enough that the fit is safe. Turn it ON 100%.
          else {
              active_scat = conformal_scat;
          }
          
          if (conformal_scat <= 1e8 * a_prime_over_a && active_scat > 0.0) {
              static int printed = 0;
              if (printed == 0) {
                  printf("\n========================================================\n");
                  printf("--> Physical Reality Check: Scattering explicitly activated at a = %e\n", a);
                  printf("========================================================\n\n");
                  printed = 1;
              }
          }*/
          ...
          dy[pv->index_pt_shear_ur] =
            0.5*(
                 ...
                 -(decay_damp+conformal_scat)*y[pv->index_pt_shear_ur]; /* NEW: Relativistic neutrino decay */
          ...
          dy[pv->index_pt_l3_ur] = k/(2.*l+1.)*
            ...
            -(27./4.*decay_damp+conformal_scat)*y[pv->index_pt_l3_ur]; /* NEW: Relativistic neutrino decay */
            //-pow(a,3)*pow(1073,3)*pow(10,6); // Escudero et al.
          ...
          for (l = 4; l < pv->l_max_ur; l++) {
            ...
            dy[pv->index_pt_delta_ur+l] = k/(2.*l+1)*
              ...
              -(alpha_l*decay_damp+conformal_scat)*y[pv->index_pt_delta_ur+l];  /* NEW: Relativistic neutrino decay */
              //-pow(a,3)*pow(1073,3)*pow(10,6); // Escudero et al.              
          }
