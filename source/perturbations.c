...
int perturbations_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    if (pba->has_ur == _TRUE_) {
      ...
      /* NEW: Relativistic neutrino decay parameters: */
      double m, conv_fact, YineV, Phi, k_fit, rho_nu_by_rho_total;
      double Mterm, g, rho_phi_by_rho_total, log_a, Mratio;
      ...
      if (ppt->has_nu_scattering == _TRUE_) {
        ...
        class_test(m*m <= ppt->delmsq,
                   error_message,
                   "nu_scattering is switched on but capX=%e (m=%e eV) is below the kinematic threshold m > sqrt(delmsq)=sqrt(%e)=%e eV required by the scattering rate fit (sqrt(m^2-delmsq) would be imaginary). Either raise capX, lower delmsq, or set nu_scattering = no.",
                   capX, m, ppt->delmsq, sqrt(ppt->delmsq));
      ...
      /* NEW: curlyF only evaluated if the decay channel is switched on. This formula itself
         ...
         too keeps decay's "off" state explicit and independent of capX's numeric value,
         mirroring how nu_scattering is gated above. */      
      if (ppt->has_nu_decay == _TRUE_ && capX > 0.0){
  ...
  curlyF = exp(-pow(a*capX,4)/16) * (- 1./2. + 2.*a*capX + (0.57721+log(a*capX))*(pow(a*capX,2)/2-1) - pow(a*capX,2) - pow(a*capX,3)/9 + 
        ...
        if(ppw->approx[ppw->index_ap_ufa] == (int)ufa_off) {
          ...
          /* NEW: conformal_scat is already 0 unless has_nu_scattering==_TRUE_ (scat_tot
             was gated above). decay_damp is explicitly gated here on has_nu_decay so its
    ...
    fprintf(rate_log, "%e %e %e %e\n", a, decay_damp, conformal_scat, ratio);
          ...
          dy[pv->index_pt_shear_ur] =
            0.5*(
                 ...
                 -(decay_damp+conformal_scat)*y[pv->index_pt_shear_ur]; /* NEW: Relativistic neutrino decay */ /* NEW: Relativistic neutrino decay */
          ...
          dy[pv->index_pt_delta_ur+l] =
           ...
           // -pow(a,3)*pow(1073,3)*pow(10,6); // Escudero et al. <- stop here for new mods
