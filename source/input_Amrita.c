...
int input_read_parameters_species(struct file_content * pfc,
  ...
  /* Some blah parameters*/
  
  /*class_call(parser_read_double(pfc,"blah1",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  if (flag1 == _TRUE_){
    pba->blah1 = param1;
    printf("Check: Read blah1 = %g\n", pba->blah1);
    }*/
    
  /*class_call(parser_read_double(pfc,"blah2",&param2,&flag2,errmsg),
             errmsg,
             errmsg);
  if (flag2 == _TRUE_){
    pba->blah2 = param2;
    printf("Check: Read blah2 = %g\n", pba->blah2);
    }
    
  if (flag1 == _TRUE_ && flag2 == _TRUE_){
  double sum = pba->blah1 + pba->blah2;
  printf("DEBUG: blah1 + blah2 = %g\n", sum);
  }*/
  ...
  printf("Assuming relativistic neutrino decay with capY=%g Mpc and capX=%g \n", ppt->capY, ppt->capX);  
    ...
    //NEW: independent on/off switches for decay and scattering damping terms
    // Set explicity with nu_decay and nu_scattering params in the .ini/.yaml files
    //default = True, elsewise specifed
    class_call(parser_read_string(pfc, "nu_decay", &string1, &flag1,errmsg),
        errmsg, 
        errmsg);
    if (flag1 == _TRUE_){
        if (string_begins_with(string1,'y') || string_begins_with(string1,'Y')){
            ppt->has_nu_decay = _TRUE_;
        }
        else {
        ...
        ppt->has_nu_decay = (ppt->capY>0.)? _TRUE_ : _FALSE_;
    }

    class_call(parser_read_string(pfc, "nu_scattering", &string1, &flag1,errmsg),
        errmsg, 
        errmsg);
    if (flag1 == _TRUE_){
        if (string_begins_with(string1,'y') || string_begins_with(string1,'Y')){
            ppt->has_nu_scattering = _TRUE_;
        }
        else {
        ...
        ppt->has_nu_scattering = (ppt->capY>0.)? _TRUE_ : _FALSE_;
    }

    //NEW: mass-squared splitting term entering decay/scattering rates [eV^2]
    // Previously hard-coded to 7.5e05, now .ini settable (default: solar splitting)
    class_call(parser_read_double(pfc, "delmsq", &param1,&flag1,errmsg),
        errmsg,
        errmsg);
    if (flag1 == _TRUE_){
        ppt->delmsq = param1;
        class_test(ppt->delmsq <=0.,
            errmsg,
